...
public class AcceptCommand {
    ...
    private static int acceptTeam(CommandSourceStack source, String teamName) throws CommandSyntaxException {
        ...
        switch (SkyblockHooks.onAccept(player, team)) {
            case DENY:
                source.sendSuccess(new TranslatableComponent("skyblockbuilder.command.denied.accept_invitations").withStyle(ChatFormatting.RED), false);
                return 0;
            case DEFAULT:
                if (!ConfigHandler.Utility.selfManage && !source.hasPermission(2)) {
                    source.sendSuccess(new TranslatableComponent("skyblockbuilder.command.disabled.accept_invitations").withStyle(ChatFormatting.RED), false);
                    return 0;
                }
                break;
            case ALLOW:
        ...
        if (!data.acceptInvite(team, player)) {
            source.sendSuccess(new TranslatableComponent("skyblockbuilder.command.error.accept_invitations").withStyle(ChatFormatting.RED), false);
            return 0;
        }

        source.sendSuccess(new TranslatableComponent("skyblockbuilder.command.success.joined_team", team.getName()).withStyle(ChatFormatting.GOLD), true);
        return 1;
    }
